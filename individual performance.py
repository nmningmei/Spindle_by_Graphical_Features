...
signal_features_indivisual_results = {}
graph_features_indivisual_results = {}
for directory_1 in [f for f in os.listdir(file_dir) if ('epoch_length' in f)]:
    ...
    for sub_fold in os.listdir(sub_dir):
        ...
        try:
            signal_features_indivisual_results[directory_1][sub_fold] = eegPipelineFunctions.cross_validation_pipeline(signal_features)
            graph_features_indivisual_results[directory_1][sub_fold] = eegPipelineFunctions.cross_validation_pipeline(df_combine)
            print(sub_fold,Counter(label),'enough samples??')
            print('signal:%.2f +/-%.2f'%(np.mean([v for (v,_,_,_) in signal_features_indivisual_results[directory_1][sub_fold]]),np.std([v for (v,_,_,_) in signal_features_indivisual_results[directory_1][sub_fold]])))
            print('graph:%.2f +/-%.2f'%(np.mean([v for (v,_,_,_) in graph_features_indivisual_results[directory_1][sub_fold]]),np.std([v for (v,_,_,_) in graph_features_indivisual_results[directory_1][sub_fold]])))
            #print(confusion_matrix())
        except:
            try:
                cv = KFold(n_splits=5,shuffle=True,random_state=0)
                signal_features_indivisual_results[directory_1][sub_fold] = eegPipelineFunctions.cross_validation_pipeline(signal_features,cv=cv)
                graph_features_indivisual_results[directory_1][sub_fold] = eegPipelineFunctions.cross_validation_pipeline(df_combine,cv=cv)
                print(sub_fold,Counter(label),'too few samples??')
            except:
                print(sub_fold,Counter(label),'not enough samples')
pickle.dump(signal_features_indivisual_results,open(file_dir+'individual_signal_feature_only.p','wb'))
pickle.dump(graph_features_indivisual_results,open(file_dir+'individual_graph_feature_only.p','wb'))